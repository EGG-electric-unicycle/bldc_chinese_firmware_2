...
GPIO_InitStructure.GPIO_Mode=GPIO_Mode_AIN; //该脚为AN模式
...
DAC_Init(DAC_Channel_1,&DAC_InitStructure); //以参数初始DAC1
DAC_Cmd(DAC_Channel_1,ENABLE); //使能DAC1通
...
{
	DAC_Ch1_Config(); //DAC1配置
	DAC_DATA=0X05FF; //DAC要写的数据
	DAC1_SET(0X05FF); //写入数据DAC转换
}
//while(1)
//	
//	{
